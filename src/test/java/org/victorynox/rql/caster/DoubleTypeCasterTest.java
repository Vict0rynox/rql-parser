...
class DoubleTypeCasterTest {

	private DoubleTypeCaster typeCaster;

	@BeforeEach
	void startUp()
	{
		typeCaster = new DoubleTypeCaster();
	}
	...
	static private Stream<Arguments> getTypeCastData()
	{
		return Stream.of(
				...
				Arguments.of(new Token(T_DATE, "2017-10-10T10:12:01", 0, 6), 2017_10_10_10_12_01.),
	...
	void typeCast(Token token, Double expectedResult) {
		Double result = typeCaster.typeCast(token);
		assertEquals(expectedResult, result);
	}

	@Test
	void getType() {
		assertEquals(typeCaster.getType(), Double.class);
	}
}