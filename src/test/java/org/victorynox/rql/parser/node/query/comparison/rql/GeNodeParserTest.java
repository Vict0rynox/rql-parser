...
class GeNodeParserTest extends AbstractNodeParserTest<GeNode<ScalarValue>> {
	...
	protected static Stream<TokenStreamIterator> getParseSyntaxErrorExceptionData() {
		...
		tokenList = new ArrayList<>();
		tokenList.add(new Token(TokenType.T_OPERATOR, "ge", 0, 6));
		tokenList.add(new Token(TokenType.T_OPEN_PARENTHESIS, "(", 6, 7));
		tokenList.add(new Token(TokenType.T_STRING, "field1", 7, 13));
		tokenList.add(new Token(TokenType.T_CLOSE_PARENTHESIS, ")", 13, 14));
		streamBuilder.add((new TokenStream(tokenList)).iterator());
	...
	protected static Stream<Arguments> getParseSuccessData() {
		...
		List<Token> tokenList;
		GeNode<ScalarValue> expectedNode;
		...
		expectedNode = new GeNode<>("field", new ScalarValue<>(-302));
		...
		expectedNode = new GeNode<>("field", new ScalarValue<>(15));
		streamBuilder.add(Arguments.of((new TokenStream(tokenList)).iterator(), expectedNode));
		...
		expectedNode = new GeNode<>("field", new ScalarValue<>(15.42));
		streamBuilder.add(Arguments.of((new TokenStream(tokenList)).iterator(), expectedNode));
	...
	@Override
	protected void parseResultAssert(GeNode expectedResult, GeNode result) {
	...
	protected void startUp() {
		...
		parser = new GeNodeParser<>(new FieldParser(), new ScalarParser(typeCasterMap));
	}
}